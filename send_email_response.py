...
This version accepts credentials as command line arguments
"""
...
def find_response_in_content(content):
    ...
    if custom_response_pos != -1:
        ...
        start_pos = content.find("\n", custom_response_pos)  # Skip the marker line
        if start_pos != -1:
            ...
            for line in lines:
                ...
                elif not line.startswith('#') and not line.startswith('**From:') and not line.startswith('---'):
                    response_lines.append(line)
                else:
                    break
    ...
    for email_file in email_files:
        ...
        if response:
            ...
            if recipient_email:
                ...
                # Send the reply using provided credentials
                success = send_reply(
                    sender_email,
                    sender_password,
                    recipient_email,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Auto-Reply System with command line credentials')
    ...
    print("Auto-Reply System (with command line credentials)")
    print("=" * 50)
